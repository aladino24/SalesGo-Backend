...
<body>
<aside class="release-sidebar"><strong style="font-size:19px">SalesGo Admin</strong><p style="font-size:11px;opacity:.8;line-height:1.5"><?php echo e($actor?->name); ?><br>Role IT</p><nav><a href="<?php echo e(route('internal.master-data.index')); ?>">Master Data</a><a class="active" href="<?php echo e(route('internal.app-releases.index')); ?>">Upload APK</a></nav><div class="release-user"><?php echo e($actor?->employee_code); ?></div></aside><div class="release-shell"><header><h1>Rilis APK Internal SalesGo</h1><p>Unggah artefak Android yang telah ditandatangani. File disimpan privat; aplikasi mobile mengunduhnya hanya setelah verifikasi MD5.</p></header>
<main>
    <section class="card">
        ...
        <?php if($errors->any()): ?><div class="alert errors"><strong>Upload belum berhasil.</strong><ul><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li><?php echo e($error); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul></div><?php endif; ?>
        <form method="post" action="<?php echo e(route('internal.app-releases.store')); ?>" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid">
                <div class="field"><label for="version_name">Versi aplikasi <span style="color:var(--danger)">*</span></label><input id="version_name" name="version_name" value="<?php echo e(old('version_name')); ?>" placeholder="Contoh: 1.0.1" required><span class="hint">Tampil untuk pengguna.</span></div>
                <div class="field"><label for="version_code">Build number <span style="color:var(--danger)">*</span></label><input id="version_code" name="version_code" type="number" min="1" value="<?php echo e(old('version_code')); ?>" placeholder="Contoh: 2" required><span class="hint">Harus unik dan meningkat.</span></div>
            </div>
            <div class="field"><label for="release_notes">Keterangan rilis</label><textarea id="release_notes" name="release_notes" placeholder="Contoh: Perbaikan check-in dan katalog produk."><?php echo e(old('release_notes')); ?></textarea></div>
            <div class="checks"><label><input type="checkbox" name="is_published" value="1" <?php echo e(old('is_published', true) ? 'checked' : ''); ?>> Publikasikan sekarang</label><label><input type="checkbox" name="is_mandatory" value="1" <?php echo e(old('is_mandatory') ? 'checked' : ''); ?>> Wajib update</label></div>
    ...
    <aside class="card">
        ...
        <div class="releases">
            <?php $__empty_1 = true; $__currentLoopData = $releases; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $release): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <article class="release"><div class="badge"><?php echo e($release->version_code); ?></div><div><h3>v<?php echo e($release->version_name); ?> · <?php echo e(number_format($release->size_bytes / 1048576, 1)); ?> MB</h3><p><?php echo e($release->original_name); ?><br><?php echo e($release->created_at?->format('d M Y H:i')); ?><?php if($release->release_notes): ?><br><?php echo e($release->release_notes); ?><?php endif; ?></p><span class="tag <?php echo e($release->is_published ? 'published' : 'draft'); ?>"><?php echo e($release->is_published ? 'Published' : 'Draft'); ?></span><?php if($release->is_mandatory): ?><span class="tag mandatory">Wajib update</span><?php endif; ?></div></article>
...
<?php /**PATH D:\SalesGo\SalesGo-Api\resources\views\internal-app-releases\index.blade.php ENDPATH**/ ?>